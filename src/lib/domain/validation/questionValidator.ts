...
export function validateQuestion(
  ...
  if (![1, 2, 3, 4].includes(q.difficulty)) {
    problems.push(
      `difficulty ${q.difficulty} is outside the supported 1-4 band`,
    );
  }
  ...
  if (!skillsById[q.skillNodeId]) {
    problems.push(
      `skillNodeId ${q.skillNodeId} does not exist in the skill hierarchy`,
    );
  }
  if (
    !q.estimatedTimeSeconds ||
    q.estimatedTimeSeconds < 10 ||
    q.estimatedTimeSeconds > 300
  ) {
    problems.push(
      `estimatedTimeSeconds ${q.estimatedTimeSeconds} looks unreasonable (expected 10-300)`,
    );
  }
  ...
  return {
    valid: true,
    status: "VALIDATED",
    notes: "passed all structural checks",
  };
}
