...
export function detectRootCauses(
  ...
  for (const skill of skills) {
    ...
    if (skill.prerequisiteSkillId) {
      ...
      const skillIsWeak =
        overall.attempts >= MIN_ATTEMPTS_FOR_CLAIM &&
        overall.accuracy !== null &&
        overall.accuracy < WEAK_THRESHOLD;
      ...
      if (skillIsWeak) {
        ...
        if (prereqSkill && prereqIsWeak) {
          ...
          if (prereqSkill.prerequisiteSkillId) {
            const grandparentSkill =
              skillsById[prereqSkill.prerequisiteSkillId];
            const grandparentState =
              state.skills[prereqSkill.prerequisiteSkillId];
