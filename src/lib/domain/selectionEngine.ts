...
export function selectNextQuestion(
  ...
  return {
    ...
    captureConfidence: shouldCaptureConfidence(
      best.purpose,
...
function scoreCandidate(
  ...
  if (!skill) {
    return {
      ...
      score: -1,
      purpose: "COVERAGE",
      rationale: "Skill metadata missing for this question.",
    };
  }
  ...
  const prereqFollowUp = state.pendingFollowUps.find(
    (f) =>
      f.reason === "PREREQUISITE_UNCHECKED" &&
      f.relatedSkillId === q.skillNodeId,
  );
  if (prereqFollowUp) {
  ...
  if (verifyFollowUp) {
    const lastAttempt =
      skillState && skillState.attempts.length > 0
        ? skillState.attempts[skillState.attempts.length - 1]
        : undefined;
  ...
  const lastDifficulty =
    skillState.attempts[skillState.attempts.length - 1]?.difficulty ??
    q.difficulty;
  const purpose: QuestionPurpose =
    q.difficulty > lastDifficulty
...
function countDomainAttempts(
  ...
  for (const [skillId, skillState] of Object.entries(state.skills)) {
    if (skillsById[skillId]?.domain === domain)
      count += skillState.attempts.length;
  }
  return count;
}
...
function nextDifficultyFor(state: SkillEvidenceState): 1 | 2 | 3 | 4 {
  ...
  if (!last) return 2;
  if (last.status !== "ANSWERED" || last.correct === null)
    return last.difficulty; // hold difficulty after a skip/don't-know
  if (last.correct) return Math.min(4, last.difficulty + 1) as 1 | 2 | 3 | 4;
  return Math.max(1, last.difficulty - 1) as 1 | 2 | 3 | 4;
}

function applicationOrderScore(
  type: ApplicationType,
  state: SkillEvidenceState,
): number {
  const foundationTouched = state.foundation.attempts >= 1;
  const foundationSolid =
    state.foundation.accuracy !== null && state.foundation.accuracy >= 0.5;
  const applicationTouched = state.application.attempts >= 1;
  const applicationSolid =
    state.application.accuracy !== null && state.application.accuracy >= 0.5;
 ...
 */
function shouldCaptureConfidence(
  purpose: QuestionPurpose,
  questionsAttemptedSoFar: number,
): boolean {
  if (purpose === "VERIFICATION" || purpose === "PREREQUISITE_CHECK")
    return true;
...
export function evaluateStoppingCondition(
  ...
  if (state.questionsAttempted >= MAX_QUESTIONS) {
    return {
      stop: true,
      reason: "Reached the maximum question count for this diagnostic.",
    };
  }
  if (state.questionsAttempted >= totalAvailableQuestions) {
    return {
      stop: true,
      reason: "Every available question in the bank has been used.",
    };
  }
  ...
  const allDomainsCovered = REQUIRED_DOMAINS.every(
    (d) => domainSkillCounts[d].size >= MIN_SKILLS_PER_DOMAIN,
  );
  ...
  if (allDomainsCovered && noPendingFollowUps) {
    return {
      stop: true,
      reason:
        "Minimum coverage reached across all domains with no unresolved follow-ups.",
    };
  }
  return { stop: false, reason: null };
}
