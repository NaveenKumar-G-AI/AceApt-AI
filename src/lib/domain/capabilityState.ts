...
function levelFromAttempts(attempts: AttemptRecord[]): SubEvidence {
  ...
  const answered = attempts.filter(
    (a) => a.status === "ANSWERED" && a.correct !== null,
  );
  ...
  const avgResponseTimeMs =
    attempts.length > 0
      ? Math.round(
          attempts.reduce((sum, a) => sum + a.responseDurationMs, 0) /
            attempts.length,
        )
      : null;
...
function isVerified(attempts: AttemptRecord[], overall: SubEvidence): boolean {
  ...
  const verificationAttempts = attempts.filter(
    (a) =>
      (a.purpose === "VERIFICATION" || a.purpose === "PREREQUISITE_CHECK") &&
      a.status === "ANSWERED" &&
      a.correct !== null,
  );
  if (verificationAttempts.length === 0) return false;
  const positiveTrend = overall.accuracy >= 0.6;
  return verificationAttempts.some((a) =>
    positiveTrend ? a.correct === true : a.correct === false,
  );
}
...
export function computeCapabilityState(
  ...
  for (const skill of skills) {
    const attempts = (attemptsBySkill[skill.id] ?? []).sort(
    ...
    const foundation = levelFromAttempts(
      attempts.filter((a) => a.applicationType === "FOUNDATION"),
    );
    const application = levelFromAttempts(
      attempts.filter((a) => a.applicationType === "APPLICATION"),
    );
    const transfer = levelFromAttempts(
      attempts.filter((a) => a.applicationType === "TRANSFER"),
    );
  ...
  for (const skill of skills) {
    ...
    const hasVerification = state.attempts.some(
      (a) => a.purpose === "VERIFICATION",
    );
    if (state.lastAttemptCorrect === false && !hasVerification) {
      pendingFollowUps.push({
        skillId: skill.id,
        reason: "UNVERIFIED_INCORRECT",
        relatedSkillId: null,
      });
    }
  ...
  return {
    ...
    questionsAnswered: history.filter((h) => h.response.status === "ANSWERED")
      .length,
    pendingFollowUps,
    askedQuestionIds,
  };
