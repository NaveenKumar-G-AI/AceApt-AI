...
export function buildDiagnosticResult(
  ...
  const domainResults: DomainResult[] = DOMAINS.map((domain) =>
    computeDomainResult(domain, state, skillsById),
  );

  const strengths = skillResults
    .filter(
      (s) =>
        s.overall.attempts >= MIN_ATTEMPTS_FOR_CLAIM &&
        s.overall.accuracy !== null &&
        s.overall.accuracy >= STRENGTH_THRESHOLD,
    )
  ...
  const focusAreas = skillResults
    .filter(
      (s) =>
        s.overall.attempts >= MIN_ATTEMPTS_FOR_CLAIM &&
        s.overall.accuracy !== null &&
        s.overall.accuracy < FOCUS_THRESHOLD,
    )
  ...
  const unexpectedFindings = computeUnexpectedFindings(
    onboardingContext,
    domainResults,
  );
  const confidenceAlignment = computeConfidenceAlignment(state, skillsById);

  const answeredAll = Object.values(state.skills)
    .flatMap((s) => s.attempts)
    .filter((a) => a.status === "ANSWERED" && a.correct !== null);
  const accuracyOverall =
    answeredAll.length > 0
      ? answeredAll.filter((a) => a.correct).length / answeredAll.length
      : null;
  const speedProfileOverall = classifySpeed(answeredAll);

  const overallCapability = computeOverallCapability(domainResults);
  const {
    skillId: recommendedStartingPointSkillId,
    reason: recommendedStartingPointReason,
  } = recommendStartingPoint(focusAreas, possibleRootCauses, skillsById);
...
function computeDomainResult(
  ...
  const totalAttempts = domainSkillStates.reduce(
    (sum, s) => sum + s.overall.attempts,
    0,
  );
  const answered = domainSkillStates
    .flatMap((s) => s.attempts)
    .filter((a) => a.status === "ANSWERED" && a.correct !== null);
...
function classifySpeed(
  answered: { responseDurationMs: number; estimatedTimeSeconds: number }[],
): DomainResult["speedProfile"] {
  if (answered.length < 2) return "INSUFFICIENT_EVIDENCE";
  const ratios = answered.map(
    (a) => a.responseDurationMs / (a.estimatedTimeSeconds * 1000),
  );
...
function computeOverallCapability(domainResults: DomainResult[]) {
  ...
  const totalAttempts = domainResults.reduce(
    (s, d) => s + d.questionsAttempted,
    0,
  );
  if (assessed.length === 0) return deriveEvidenceLevel(null, 0);
  const avgAccuracy =
    assessed.reduce((s, d) => s + (d.accuracy ?? 0), 0) / assessed.length;
 ...
 */
function computeUnexpectedFindings(
  ...
  for (const [skillId, skillState] of Object.entries(state.skills)) {
    for (const attempt of skillState.attempts) {
      ...
      const highConf =
        ...
        attempt.confidence === "VERY_CONFIDENT";
      const lowConf =
        attempt.confidence === "GUESSING" || attempt.confidence === "NOT_SURE";
...
function recommendStartingPoint(
  ...
  return {
    skillId: null,
    reason:
      "No clear focus area emerged from this diagnostic — broader practice across all three domains is a reasonable next step.",
  };
}
