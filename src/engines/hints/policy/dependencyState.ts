...
export function computeDependencyState(
  ...
  recentEscalationFlags: boolean[],
): DependencyState {
  if (recentHintLevels.length < MIN_SAMPLES_FOR_A_LABEL)
    return DependencyState.UNKNOWN;

  const avgLevel =
    recentHintLevels.reduce((a, b) => a + b, 0) / recentHintLevels.length;
  const escalationRate =
    recentEscalationFlags.length > 0
      ? recentEscalationFlags.filter(Boolean).length /
        recentEscalationFlags.length
      : 0;
...
export type FadingTrend =
  "DECREASING" | "STABLE" | "INCREASING" | "NOT_ENOUGH_DATA";
