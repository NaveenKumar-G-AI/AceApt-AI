 ...
 */
const DEFAULT_SEQUENCE: StrategyTag[] = [
  "DIRECT_CLUE",
  "EXAMPLE",
  "DECOMPOSITION",
  "WORKED_STEP",
...
function clampLevel(
  n: number,
  min = HintLevel.L0_NONE,
  max = HintLevel.L7_COMPLETE_SOLUTION,
): HintLevel {
...
function denied(
  req: PolicyRequest,
  reason: PolicyDecision["denialReason"],
): PolicyDecision {
  return {
    shouldOffer: false,
    denialReason: reason,
...
export function diagnoseBlockType(req: PolicyRequest): BlockType {
  if (req.explicitConfidenceSelfReport === "UNSURE_DESPITE_CORRECT")
    return BlockType.CONFIDENCE_BLOCK;

  if (!req.attempt) {
    // No submission yet on this step at all.
    return req.attemptCountOnStep === 0
      ? BlockType.STARTING_POINT_BLOCK
      : BlockType.UNKNOWN;
  }

  switch (req.attempt.mistakeSignal) {
    case MistakeSignal.NO_ATTEMPT:
      return req.trigger === TriggerType.EXPLICIT_CONFUSION
        ? BlockType.CONCEPT_BLOCK
        : BlockType.STARTING_POINT_BLOCK;
...
export function decide(req: PolicyRequest): PolicyDecision {
  ...
  if (
    req.assessmentMode === AssessmentMode.ASSESSMENT &&
    !req.hintsExplicitlyPermittedInAssessment
  ) {
    return denied(req, "ASSESSMENT_DISABLED"); // §32, §72
  }
  if (
    req.budget?.maxHints !== undefined &&
    req.priorHintsThisStep.length >= req.budget.maxHints
  ) {
    return denied(req, "BUDGET_EXCEEDED"); // §35–36
  }
  const lastOutcome =
    req.priorOutcomesThisStep[req.priorOutcomesThisStep.length - 1];
  ...
  if (
    req.trigger === TriggerType.STUCKNESS ||
    req.trigger === TriggerType.LONG_STALL
  ) {
    const stallRatio =
      req.medianTimeForStepMs > 0
        ? req.timeOnStepMs / req.medianTimeForStepMs
        : 1;
    const evidenceEnough =
      stallRatio >= STALL_RATIO_THRESHOLD ||
      req.attemptCountOnStep >= MIN_ATTEMPTS_FOR_STUCKNESS;
  ...
  if (req.attempt?.mistakeSignal === MistakeSignal.GUESS_CORRECT) {
    ...
    return {
      ...
      revealsAnswer: false,
      rationale:
        "Correct, but the attempt pattern looks like a guess rather than reasoning.",
      maxWords: 0,
  ...
  const isUnitCheck =
    req.attempt?.mistakeSignal === MistakeSignal.UNIT_ERROR ||
    req.explicitConfidenceSelfReport === "CHECKING_UNIT_ONLY";
  if (isUnitCheck) {
  ...
  // of the same one.
  const failuresThisTypeOnStep = req.priorOutcomesThisStep.filter(
    (outcome, i) => {
      const interaction = req.priorHintsThisStep[i];
      return (
        outcome.result === HintOutcomeResultEnum.NO_EFFECT &&
        interaction?.hintType === hintType
      );
    },
  ).length;
  ...
  if (strategyTag === "WORKED_STEP") {
    ...
    level = clampLevel(
      level + Math.max(failuresThisTypeOnStep, streakEscalation),
      HintLevel.L0_NONE,
      HintLevel.L5_STEP_GUIDANCE,
    );
    ...
    if (req.dependencyState === DependencyState.HIGH)
      level = clampLevel(
        level + 1,
        HintLevel.L0_NONE,
        HintLevel.L5_STEP_GUIDANCE,
      );
    if (req.dependencyState === DependencyState.LOW)
      level = clampLevel(
        level - 1,
        HintLevel.L1_DIRECTIONAL,
        HintLevel.L5_STEP_GUIDANCE,
      );
  }
  ...
  const maxAllowed =
    req.budget?.maxLevel ??
    (req.requestFullSolution
      ? HintLevel.L7_COMPLETE_SOLUTION
      : HintLevel.L6_WORKED_STEP);
