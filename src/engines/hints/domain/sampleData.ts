...
const percentage: TrustedQuestion = {
  ...
  difficulty: "EASY",
  prompt:
    "The price of an item increases from ₹500 to ₹600. Find the percentage increase.",
  finalAnswer: "20%",
  solutionSteps: [
    {
      stepId: "strategy",
    ...
    },
    {
      ...
      explanation:
        "Increase = 600 − 500 = 100. Divide by the original value, 500, then multiply by 100.",
      correctValue: "20",
    },
  ],
  classifyMistake(stepId, normalized) {
    if (stepId === "strategy") {
      if (normalized === "opt_unsure" || normalized === "")
        return MistakeSignal.NO_ATTEMPT;
...
const probability: TrustedQuestion = {
  ...
  solutionSteps: [
    ...
    },
    {
      ...
      explanation:
        "6 favourable outcomes out of 45 total outcomes simplifies to 2/15 (≈ 0.1333).",
      correctValue: "2/15",
    },
  ],
  classifyMistake(stepId, normalized) {
    ...
    if (normalized === "") return MistakeSignal.NO_ATTEMPT;
    const n = normalized.includes("/")
      ? evalFraction(normalized)
      : num(normalized);
    if (Number.isNaN(n)) return MistakeSignal.CALCULATION_ERROR;
...
export const SAMPLE_QUESTIONS: TrustedQuestion[] = [
  percentage,
  probability,
  puzzle,
];
