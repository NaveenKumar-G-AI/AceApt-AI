import type { EvidenceTrace, SkillEstimate } from "../types/domain";
...
export function buildSupportingEvidence(
  ...
  responses: EvidenceTraceResponsePoint[],
): string[] {
  ...
  lines.push(
    `${countable.length} questions answered independently, ${correct} correct and ${countable.length - correct} incorrect.`,
  );

  const slowFraction =
    countable.filter(
      (r) =>
        r.expectedDurationMs > 0 && r.durationMs / r.expectedDurationMs > 1.5,
    ).length / countable.length;
  if (slowFraction > 0.4) {
    lines.push("Response time ran above expected on several of these.");
  }

  const highConfidenceWrong = countable.filter(
    (r) => !r.isCorrect && (r.confidence ?? 0) >= 4,
  ).length;
  if (highConfidenceWrong > 0) {
    lines.push(
      `${highConfidenceWrong} incorrect response(s) came with high stated confidence.`,
    );
  }

  if (estimate.consistencyFlag) {
    lines.push(
      "Performance varied noticeably between attempts rather than trending in one direction.",
    );
  }

  if (estimate.confidenceState === "low") {
    lines.push(
      "This is an early read — more independent evidence would sharpen it.",
    );
  }
