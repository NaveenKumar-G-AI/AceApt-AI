import type { DifficultyBoundary, QuestionDifficulty } from "../types/domain";
...
export function computeDifficultyProfile(
  scopeNodeId: string,
  responses: DifficultyResponsePoint[],
): DifficultyBoundary {
  ...
  for (const band of DIFFICULTY_ORDER) {
    ...
    if (subset.length >= MIN_PER_BAND) {
      accuracyByDifficulty[band] = Number(
        (subset.filter((r) => r.isCorrect).length / subset.length).toFixed(2),
      );
    }
  }

  const present = DIFFICULTY_ORDER.filter(
    (b) => accuracyByDifficulty[b] !== undefined,
  );
  let boundaryDetected = false;
  let interpretation =
    "Not enough spread across difficulty levels yet to identify a clear breakdown point.";

  for (let i = 0; i < present.length - 1; i++) {
    const lowerBand = present[i]!;
    const higherBand = present[i + 1]!;
    const drop =
      accuracyByDifficulty[lowerBand]! - accuracyByDifficulty[higherBand]!;
  ...
  if (!boundaryDetected && present.length >= 2) {
    interpretation =
      "Performance holds fairly steady across the difficulty levels answered so far.";
  }

  return {
    domainOrTopicNodeId: scopeNodeId,
