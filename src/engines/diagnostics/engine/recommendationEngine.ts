...
function actionForStatus(estimate: SkillEstimate): {
  action: string;
  ...
  switch (estimate.status) {
    case "needs_focus":
      return {
        ...
        priority: estimate.confidenceState === "low" ? "medium" : "high",
      };
    case "developing":
...
function rationaleFor(estimate: SkillEstimate, isBottleneck: boolean): string {
  if (
    estimate.confidenceState === "incomplete" ||
    estimate.confidenceState === "low"
  ) {
...
export function buildRecommendations(
  ...
  const bottleneckSkillIds = new Set(
    bottlenecks.flatMap((b) => b.possibleBottleneckOf),
  );

  const leafEstimates = skillEstimates.filter(
    (e) => e.nodeLevel === "skill" && e.status !== "strong",
  );
