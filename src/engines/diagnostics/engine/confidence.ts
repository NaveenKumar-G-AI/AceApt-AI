...
export function assessConfidenceCalibration(
  ...
  const highConfidenceWrongRate =
    highConf.length > 0 ? highConfidenceWrongCount / highConf.length : null;
  const lowConfidenceCorrectRate =
    lowConf.length > 0 ? lowConfidenceCorrectCount / lowConf.length : null;

  const overconfident =
    highConf.length >= MIN_BUCKET_EVIDENCE &&
    (highConfidenceWrongRate ?? 0) > OVERCONFIDENCE_THRESHOLD;
  const underconfident =
    lowConf.length >= MIN_BUCKET_EVIDENCE &&
    (lowConfidenceCorrectRate ?? 0) > UNDERCONFIDENCE_THRESHOLD;
