...
import { descendantSkills, nodeById } from "./blueprint";
...
export function estimateFromEvidence(
  ...
  const weightedCorrect = countable
    .filter((e) => e.isCorrect)
    .reduce((s, e) => s + e.evidenceWeight, 0);
  const weightedTotal = countable.reduce((s, e) => s + e.evidenceWeight, 0);
  const pointEstimate =
    (priorStrength * priorMean + weightedCorrect) /
    (priorStrength + weightedTotal);
  return {
    pointEstimate,
    weightedEvidence: weightedTotal,
    rawCount: countable.length,
  };
}
...
export function estimateNode(
  ...
  const consistencyFlag = leaves.some(
    (leaf) => consistencyFlagBySkill.get(leaf.id) === true,
  );

  const { pointEstimate, weightedEvidence, rawCount } =
    estimateFromEvidence(pooled);
  const confidenceState = deriveConfidenceState(
    rawCount,
    node.minEvidenceCount,
    consistencyFlag,
  );
...
export function estimateAllNodes(
  ...
  return blueprint.nodes.map((node) =>
    estimateNode(blueprint, node, evidenceBySkill, consistencyFlagBySkill),
  );
}
