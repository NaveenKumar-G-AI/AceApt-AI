import type { ConsistencyResult } from "../types/domain";
...
export function analyzeConsistency(
  skillNodeId: string,
  chronologicalCorrectness: boolean[],
): ConsistencyResult {
  if (chronologicalCorrectness.length < CHUNK_SIZE * 2) {
    return {
      ...
      isConsistent: true,
      chunkAccuracies: [],
      note: "insufficient evidence to assess consistency",
    };
  }
  ...
  const chunkAccuracies = chunks.map((c) =>
    Number((c.filter(Boolean).length / c.length).toFixed(2)),
  );
  const max = Math.max(...chunkAccuracies);
  const min = Math.min(...chunkAccuracies);
  const spread = max - min;
  const flaggedInconsistent =
    spread > SPREAD_THRESHOLD && max >= HIGH_BAND && min <= LOW_BAND;
