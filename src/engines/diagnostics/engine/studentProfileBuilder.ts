...
import { descendantSkills } from "./blueprint";
import {
  estimateAllNodes,
  deriveStatus,
  type SkillEvidencePoint,
...
import { computeAccuracyProfile } from "./accuracyProfile";
import { computeDifficultyProfile } from "./difficultyProfile";
import { detectBottlenecks } from "./bottleneckDetection";
import { buildEvidenceTrace } from "./explainability";
...
function groupBySkill(
  responses: ProfileResponsePoint[],
): Map<string, ProfileResponsePoint[]> {
...
function pooledForNode(
  blueprint: Blueprint,
  nodeId: string,
  bySkill: Map<string, ProfileResponsePoint[]>,
): ProfileResponsePoint[] {
  return descendantSkills(blueprint, nodeId).flatMap(
    (leaf) => bySkill.get(leaf.id) ?? [],
  );
}
...
export function buildStudentProfile(
  ...
  for (const [skillId, rs] of bySkill.entries()) {
    ...
    const chronologicalCorrectness = rs
      .filter((r) => r.isCorrect !== null)
      .map((r) => r.isCorrect as boolean);
  ...
  const allEstimates = estimateAllNodes(
    blueprint,
    evidenceBySkill,
    consistencyFlagBySkill,
  );
  const estimatesBySkillId = new Map(
    allEstimates
      .filter((e) => e.nodeLevel === "skill")
      .map((e) => [e.skillNodeId, e]),
  );

  const domainProfiles = allEstimates.filter((e) => e.nodeLevel === "domain");
  const topicProfiles = allEstimates.filter((e) => e.nodeLevel === "topic");
  const subtopicProfiles = allEstimates.filter(
    (e) => e.nodeLevel === "subtopic",
  );
  const skillProfiles = allEstimates.filter((e) => e.nodeLevel === "skill");

  const withEvidence = skillProfiles.filter(
    (e) => e.confidenceState !== "incomplete",
  );
  ...
  const speedProfile = domainProfiles.map((d) =>
    computeSpeedProfile(
      d.skillNodeId,
      pooledForNode(blueprint, d.skillNodeId, bySkill).map((r) => ({
  ...
  const confidenceCalibration: ConfidenceCalibrationEntry[] =
  ...
  const overallConfidence = domainProfiles.reduce<DiagnosticConfidenceState>(
    (worst, d) =>
      CONFIDENCE_RANK[d.confidenceState] < CONFIDENCE_RANK[worst]
        ? d.confidenceState
        : worst,
    domainProfiles.length > 0
      ? domainProfiles[0]!.confidenceState
      : "incomplete",
  );

  const evidencedDomains = domainProfiles.filter(
    (d) => d.confidenceState !== "incomplete",
  );
  const overallStatus: CapabilityStatus | "incomplete" =
    evidencedDomains.length > 0
      ? deriveStatus(
          evidencedDomains.reduce((s, d) => s + d.pointEstimate, 0) /
            evidencedDomains.length,
          overallConfidence,
        )
      : "incomplete";
