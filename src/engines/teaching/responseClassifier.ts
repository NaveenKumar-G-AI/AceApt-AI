...
const UNSURE_MARKERS =
  /\bi\s?(don'?t|do not)\s?know\b|\bnot sure\b|\bno idea\b|\bconfused\b|\bunsure\b/i;
...
export function classifyDeterministic(input: ClassifyInput): ClassifyResult {
  ...
  const misconception = KNOWN_MISCONCEPTIONS.find(
    (m) => m.skill === input.skill && m.test(text),
  );
  if (misconception) {
    return {
  ...
  const reasoningDetected =
    REASONING_MARKERS.test(text) ||
    (input.referenceKeywords ?? []).some((k) =>
      lower.includes(k.toLowerCase()),
    );

  if (allNumbers.length > 0) {
    const matches = allNumbers.some(
      (n) => Math.abs(n - input.trustedAnswer) <= tolerance,
    );
    if (matches) {
      return {
        classification: reasoningDetected
          ? "CORRECT_REASONING"
          : "CORRECT_GUESS",
        extractedNumber: num,
        matchesTrusted: true,
    ...
    const usedWrongReference = (input.wrongReferenceKeywords ?? []).some(
 ...
 * understanding).
 */
export function classifyReasoningElicitation(
  text: string,
  base: number,
): ClassifyResult {
  ...
  const hasOperationLanguage =
    /%|percent|÷|\/|×|\*|divide|multiply|original|starting|base/i.test(trimmed);
