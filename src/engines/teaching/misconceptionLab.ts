...
export function startExperiment(percent: number): MisconceptionExperimentState {
  const safePercent =
    Number.isFinite(percent) && percent > 0 && percent < 100 ? percent : 20;
  return {
    misconceptionId: "percentage_increase_decrease_cancel",
    step: 0,
    base: 100,
    percent: safePercent,
  };
...
export function evaluateStep(
  state: MisconceptionExperimentState,
  studentText: string,
): ExperimentStepEvaluation {
  ...
  if (state.step === 1) {
    ...
    return {
      ...
      hint: correct
        ? undefined
        : `Take ${state.percent}% of ${afterIncreaseDisplay} and subtract it from ${afterIncreaseDisplay}.`,
    };
  }

  if (state.step === 2) {
    const saidNo = /\bno\b|not the same|didn'?t|different|lower|less/i.test(
      studentText,
    );
  ...
  const understood =
    /base|different (number|amount|value)|changed|smaller (base|number|amount)|not the same/i.test(
      studentText,
    );
