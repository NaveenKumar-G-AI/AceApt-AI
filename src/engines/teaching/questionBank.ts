...
export function generatePercentageProblem(
  rng: () => number = Math.random,
  avoid?: ProblemContext,
): ProblemContext {
...
export function generateChangeAmountProblem(
  rng: () => number = Math.random,
  avoid?: ProblemContext,
): ProblemContext {
  let ctx: ContextWord, base: number, percent: number;
  ...
  return {
    ...
    trustedSolutionSteps: [
      `Increase = ${percent}% of ${base} = ${changeAmount}`,
    ],
...
export function generateSimplifiedVariant(
  ...
  return {
    ...
    trustedSolutionSteps: [
      `Change = ${percent}% of ${base} = ${changeAmount}`,
      `New value = ${base + changeAmount}`,
    ],
    variables: {
      base,
      percent,
      changeAmount,
      newValue: base + changeAmount,
      contextNoun,
    },
    generator: "percentage_change_new_value_v1",
  };
