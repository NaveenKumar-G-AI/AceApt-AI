...
const SLOW_STATES = new Set([
  SpeedPerformanceState.SLOW_ACCURATE,
  SpeedPerformanceState.SLOW_INACCURATE,
]);
...
function isMostlyFastOrOnPace(window: SpeedAttemptRecord[]): boolean {
  const ok = window.filter((a) =>
    NOT_SLOW_STATES.has(a.performanceState),
  ).length;
...
function rampTarget(ctx: TrainingPolicyContext): number | null {
  ...
  const floor = ctx.baseline
    ? Math.round(ctx.baseline.averageMs * MIN_TARGET_RATIO_OF_BASELINE)
    : 0;
  const stepped = Math.round(
    ctx.currentTargetMs * (1 - TARGET_RAMP_STEP_RATIO),
  );
...
export function evaluateTrainingPolicy(
  ...
  if (window.length < MIN_ATTEMPTS_FOR_DECISION) {
    return hold(
      ctx,
      "INSUFFICIENT_DATA",
      "Not enough attempts yet in this session to adjust pace.",
      [],
    );
  }
  ...
  const accuracyBelowGuardrail =
    accuracy < ctx.guardrailAccuracy - ACCURACY_DROP_TOLERANCE;
  ...
  if (accuracyBelowGuardrail && mostlyFast) {
    return {
      ...
      nextTargetMs: ctx.currentTargetMs
        ? Math.round(ctx.currentTargetMs * RUSHING_RELIEF_RATIO)
        : null,
      message:
        "Your pace increased, but accuracy dropped. Let's return to a safer pace and focus on efficient decisions.",
      evidence: [
        `Accuracy fell to ${pct(accuracy)} while your response times dropped below your usual pace.`,
      ],
  ...
  if (accuracyBelowGuardrail) {
    return {
      ...
      nextTargetMs: ctx.currentTargetMs,
      message:
        "Your accuracy needs to stabilize before we increase time pressure. Let's shore up the underlying concept first.",
      evidence: [
        `Accuracy is ${pct(accuracy)}, below your ${pct(ctx.guardrailAccuracy)} guardrail, even without unusual time pressure.`,
      ],
      requiresCoachingNarrative: true,
    };
  }
  ...
  if (mostlySlow && accuracy >= ctx.guardrailAccuracy) {
    return {
      ...
      nextTargetMs: ctx.currentTargetMs,
      message:
        "Your accuracy is strong. Let's work on reducing unnecessary decision time rather than pushing raw speed.",
      evidence: [
        "Recent responses are consistently slower than your baseline despite strong accuracy.",
      ],
      requiresCoachingNarrative: true,
    };
  }
  ...
  if (
    accuracy >= ctx.guardrailAccuracy &&
    isMostlyFastOrOnPace(window) &&
    !mostlySlow
  ) {
    const nextTarget = rampTarget(ctx);
    const rampedDown =
      nextTarget !== null &&
      ctx.currentTargetMs !== null &&
      nextTarget < ctx.currentTargetMs;
    return {
      ...
      evidence: [
        `Accuracy ${pct(accuracy)} at or above your guardrail, with pace at or ahead of target.`,
      ],
  ...
  return hold(
    ctx,
    BottleneckType.UNKNOWN,
    "Keep going - we're gathering more evidence before adjusting your pace.",
    [],
  );
}
