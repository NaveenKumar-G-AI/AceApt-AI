...
export function classifySpeedState(
  ...
  if (expectedTimeMs === null || expectedTimeMs <= 0) {
    return {
      relativeSpeed: null,
      state: SpeedPerformanceState.INSUFFICIENT_DATA,
    };
  }
...
export function computeBaseline(
  ...
  let pool = independentOnly
    ? attempts.filter((a) => a.independent && a.hintLevel === 0)
    : attempts;
...
export function resolveExpectedTime(params: {
  ...
  if (params.calibratedExpectedTimeMs && params.calibratedExpectedTimeMs > 0) {
    return {
      expectedTimeMs: params.calibratedExpectedTimeMs,
      source: "CALIBRATED",
    };
  }
  if (
    params.personalBaseline &&
    params.personalBaseline.sampleSize >= minSample
  ) {
    return {
      expectedTimeMs: params.personalBaseline.averageMs,
