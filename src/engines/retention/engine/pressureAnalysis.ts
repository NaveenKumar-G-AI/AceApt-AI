...
export async function analyzeRetrievalUnderPressure(
  ...
  if (
    evidence.timedAttemptCount < MIN_ATTEMPTS_PER_BUCKET ||
    evidence.untimedAttemptCount < MIN_ATTEMPTS_PER_BUCKET
  ) {
    return false;
  }
  if (
    evidence.timedSuccessRate === null ||
    evidence.untimedSuccessRate === null
  )
    return false;
  if (
    evidence.timedVsUntimedGap === null ||
    evidence.timedVsUntimedGap < CONCERNING_GAP
  )
    return false;
