...
export const DEFAULT_DECAY_PROFILE = (
  studentId: string,
): PersonalDecayProfile => ({
  studentId,
...
export function assessDecay(
  ...
  if (!latestAttemptCorrect) {
    delta += 1;
    if (
      evidence.baselineSuccessRate !== null &&
      evidence.recentSuccessRate !== null
    ) {
      const drop = evidence.baselineSuccessRate - evidence.recentSuccessRate;
      if (drop >= DROP.severe) {
        delta += 1;
        rationale.push(
          `Recent success is well below this concept's mastery baseline (down ~${Math.round(drop * 100)} pts).`,
        );
    ...
    if (
      evidence.transferSuccessRate !== null &&
      evidence.transferSuccessRate < 0.5
    ) {
      delta += 1;
    ...
    const transferOk =
      evidence.transferSuccessRate === null ||
      evidence.transferSuccessRate >= 0.6;
    if (
      currentIdx > 0 &&
      recentlyStrong &&
      transferOk &&
      evidence.attemptCount >= 3
    ) {
      delta -= 1;
      rationale.push(
        "Recent evidence is strong and consistent — retention risk is easing.",
      );
    }
  }
  ...
  if (staleWithoutFreshEvidence) {
    ...
    rationale.push(
      "It has been longer than this student's usual retention window with no fresh check.",
    );
  }
  ...
  if (evidence.contextDiversityScore < 0.3 && evidence.attemptCount >= 4) {
    ...
    rationale.push(
      "Success so far comes from very similar contexts — mastery may be narrower than it looks.",
    );
  }

  const nextIdx = Math.min(
    RISK_STATE_ORDER.length - 1,
