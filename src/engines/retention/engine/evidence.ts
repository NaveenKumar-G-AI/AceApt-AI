 ...
 */
export function computeContextDiversityScore(
...
export function computeEvidenceSufficiency(
  ...
  const distinctDays = new Set(attempts.map((a) => a.createdAt.slice(0, 10)))
    .size;
  const spreadFactor = Math.min(1, distinctDays / 3); // spread over time, not one sitting
  return Number(
    (0.5 * volumeFactor + 0.3 * spreadFactor + 0.2 * diversityFactor).toFixed(
      3,
    ),
  );
}
...
export function buildRetentionEvidence(
  ...
  return {
    ...
    delayDaysSinceLastSuccess: lastSuccess
      ? Number(daysBetween(lastSuccess.createdAt, now).toFixed(2))
      : null,
    ...
    timedVsUntimedGap:
      successRate(untimedAttempts) !== null &&
      successRate(timedAttempts) !== null
        ? Number(
            (
              successRate(untimedAttempts)! - successRate(timedAttempts)!
            ).toFixed(3),
          )
        : null,
    hintDependencyRate: sorted.length
      ? sorted.filter((a) => a.hintsUsed > 0).length / sorted.length
      : 0,
    explanationDependencyRate: sorted.length
      ? sorted.filter((a) => a.explanationRequested).length / sorted.length
      : 0,
