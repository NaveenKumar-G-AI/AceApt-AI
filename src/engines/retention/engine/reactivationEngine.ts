...
export type ReactivationAction =
  | {
      ...
      stepKind: ReactivationStep["kind"];
    }
...
export function recordReactivationOutcome(
  ...
  if (phase === "repair") {
    ...
    return {
      ...
      nextAction: {
        kind: "repair_step",
        level: session.currentLevel,
        stepKind: LEVEL_KIND[session.currentLevel],
      },
    };
  }
  ...
  if (phase === "similar") {
    ...
    if (succeeded)
      return { session, nextAction: { kind: "transfer_question" } };
...
export function firstRepairAction(
  session: ReactivationSession,
): ReactivationAction {
  return {
    kind: "repair_step",
