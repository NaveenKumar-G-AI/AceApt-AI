...
export function expectedValueOfAttempt(
  probabilityCorrectPercent: number,
  policy: ScoringInputs,
): number {
 ...
 */
export function isAttemptFavored(
  probabilityCorrectPercent: number,
  policy: ScoringInputs,
): boolean | null {
 ...
 */
export const ILLUSTRATIVE_BAND_PROBABILITY_RANGE: Record<
  ConfidenceBand,
  [number, number]
> = {
  VERY_LOW: [0, 20],
...
export function expectedValueRangeForBand(
  band: ConfidenceBand,
  policy: ScoringInputs,
): [number, number] {
  const [lo, hi] = ILLUSTRATIVE_BAND_PROBABILITY_RANGE[band];
  const values = [
    expectedValueOfAttempt(lo, policy),
    expectedValueOfAttempt(hi, policy),
  ];
