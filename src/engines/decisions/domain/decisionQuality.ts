...
const CONTINUE_LIKE_ACTIONS: DecisionAction[] = [
  "FULL_SOLVE",
  "PARTIAL_SOLVE",
  "CONTINUE",
];
const RISK_MITIGATING_ACTIONS: DecisionAction[] = [
  "ELIMINATE",
  "ESTIMATE",
  "SKIP",
  "RETURN_LATER",
...
function rateEvidenceUsed(
  ...
  if (evidence.length >= 3 || level === "VERIFIED" || level === "OBSERVED")
    return "HIGH";
  return "MEDIUM";
}

function rateOptionReduction(
...
function rateRiskAwareness(input: DecisionQualityInput): QualityRating {
  if (!input.policy || input.policy.source === "UNKNOWN")
    return "NOT_ASSESSABLE";
  const meaningfulPenalty = input.policy.wrongPenalty > 0;
  const uncertain =
    input.uncertaintyState === "UNCERTAIN" ||
    input.uncertaintyState === "LOW_CONFIDENCE" ||
    input.uncertaintyState === "NO_USEFUL_EVIDENCE";
...
function rateConfidenceCalibration(input: DecisionQualityInput): QualityRating {
  if (
    !input.comparableCalibration ||
    input.comparableCalibration.sampleSize <
      MIN_SAMPLE_FOR_PER_DECISION_CALIBRATION
  ) {
    return "NOT_ASSESSABLE";
  }
...
export function assessProcessQuality(
  input: DecisionQualityInput,
): DecisionQualityProfile {
  return {
    evidenceUsed: rateEvidenceUsed(input.action, input.evidenceUsed),
    optionReduction: rateOptionReduction(
      input.totalOptions,
      input.eliminatedOptionIds.length,
    ),
