...
import type { FormulaRepository } from "./repositories";
...
const speedDistanceTimeRaw: Formula = {
  ...
  meaning:
    "Distance covered equals speed multiplied by the time spent travelling at that speed.",
  variables: [
  ...
  conditions: {
    whenToUse:
      "Speed is constant over the interval, or a single average speed for the whole interval is given.",
    whenNotToUse:
...
const simpleInterestRaw: Formula = {
  ...
  conditions: {
    whenToUse:
      "Interest for each period is computed on the original principal only, and is not reinvested into a growing balance.",
    whenNotToUse:
...
const compoundInterestRaw: Formula = {
  ...
  conditions: {
    whenToUse:
      "Interest earned each year is added to the balance and itself earns interest in subsequent years.",
    whenNotToUse:
...
function withValidatedDerivedForms(formula: Formula): Formula {
  ...
  return {
    ...formula,
    derivedForms: validateAllDerivedForms(
      formula.canonicalExpression,
      formula.derivedForms,
      variableSymbols,
    ),
  };
}
...
export async function seedFormulaRepository(
  ...
  for (const formula of SEED_FORMULAS) {
    ...
    for (const form of formula.derivedForms) {
      if (form.validated === false) {
        throw new Error(
          `Seed formula "${formula.formulaId}" has an invalid derived form: ${form.expression}`,
        );
      }
    }
