...
export interface StudentStateRepository {
  getState(
    studentId: string,
    formulaId: string,
  ): Promise<FormulaStudentState | null>;
...
export interface TrainingAttemptRepository {
  ...
  listAttempts(
    studentId: string,
    filter?: { formulaId?: string },
  ): Promise<FormulaTrainingAttempt[]>;
}
...
export class InMemoryFormulaRepository implements FormulaRepository {
  ...
  async listFormulas(filter?: {
    ...
    if (filter?.tenantId !== undefined) {
      all = all.filter(
        (f) => f.tenantId === filter.tenantId || f.tenantId === undefined,
      );
    }
    return all;
  }
  ...
  async getRelationships(formulaId: string): Promise<FormulaRelationship[]> {
    ...
    return this.relationships.filter(
      (r) => r.sourceFormulaId === formulaId || r.targetFormulaId === formulaId,
    );
  }
  ...
  async listAttempts(
    ...
    return this.attempts.filter(
      (a) =>
        ...
        (!filter?.formulaId || a.formulaId === filter.formulaId),
    );
  }
}
