...
export function dimensionForActivity(
...
export function dimensionStatus(
  ...
  if (
    evidence.attempts < MIN_SAMPLES_FOR_SIGNAL ||
    evidence.recentOutcomes.length === 0
  )
    return "UNKNOWN";
  const acc =
    evidence.recentOutcomes.filter(Boolean).length /
    evidence.recentOutcomes.length;
...
function nextSupport(
  ...
  correct: boolean,
  streak: number,
): { level: SupportLevel; streak: number } {
...
export class FormulaStudentStateService {
   ...
   */
  async applyAttempt(
    attempt: FormulaTrainingAttempt,
  ): Promise<FormulaStudentState> {
    ...
    state.regressionFlag = Object.values(state.dimensions).some(
      detectDimensionRegression,
    );
  ...
  async getProfile(studentId: string): Promise<
    Array<{
      formulaId: string;
      dimensions: Partial<Record<CompetencyDimension, DimensionStatusLabel>>;
  ...
  > {
    ...
    return states.map((state) => {
      ...
      return {
        ...
        dimensions: dims,
        regressionFlag: state.regressionFlag,
      };
    });
  }
}
