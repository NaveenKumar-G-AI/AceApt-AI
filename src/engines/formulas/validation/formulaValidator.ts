...
export function parseEquation(equation: string): EquationParts {
  ...
  if (parts.length !== 2) {
    throw new Error(
      `Cannot parse equation "${equation}" - expected a single "target = expression" form.`,
    );
  }
  ...
  if (!target || !expression) {
    throw new Error(
      `Cannot parse equation "${equation}" - empty target or expression.`,
    );
  }
  ...
  for (let trial = 0; trial < TRIALS; trial++) {
    ...
    if (!Number.isFinite(computed) || !Number.isFinite(expected)) {
      return {
        valid: false,
        trialsRun: trial,
        counterexample: { scope, expected, computed },
      };
    }
...
export function validateAllDerivedForms(
  ...
  return derivedForms.map((form) => ({
    ...form,
    validated: validateDerivedForm(canonicalExpression, form, variableSymbols)
      .valid,
  }));
}
