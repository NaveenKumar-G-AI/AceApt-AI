 ...
 */
export function classifyFormulaError(
  checks: AttemptChecks,
): FormulaErrorType | null {
  ...
  if (activityType === "SELECT" && checks.formulaCorrect === false) {
    return checks.chosenFormulaRelatedToExpected
...
export function detectRetentionGap(
  ...
  if (immediateSamples < MIN_SAMPLES || delayedSamples < MIN_SAMPLES)
    return false;
